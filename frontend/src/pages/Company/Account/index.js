...
import { RegisterAccount, EditAccount, DeleteAccount } from '../../../components/Account';
...
function Account() {
  ...
  useEffect(() => {
    ...
    // SET FILTER TABLE
    if (month !== '' && year !== '') {
      api.get(`/account/${month}/${year}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      }).then(res => {
        res.data.sort((a, b) => {
          return a.dueDate < b.dueDate ? -1
  ...
  return (
    <div id="page-dashboard">
      <div className="columns">
        ...
        <div className="column container-painel">
          ...
          <div className="painel-title">
            <label htmlFor="">CONTAS Á PAGAR</label>
          </div>
            ...
            <table className="table is-fullwidth">
              ...
              <tbody>
                {array.map(dice => {
                  return (
                    <tr key={dice._id}>
                      <td>{ConvertDate(dice.dueDate)}</td>
                      ...
                      <td>
                        <div className="table-button">
                          ...
                          <button onClick={() => DeleteAccount(dice._id, array, setArray)} className="button is-danger is-medium">Excluir</button>
                        </div>
                      </td>
                    </tr>
                  )
      ...
      <RegisterAccount active={modal} close={closeModal} />
      <EditAccount active={editModal} close={closeEditModal} />
